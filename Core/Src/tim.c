...
void HALL_Get_Electrical_Angle(void *pHandleVoid)
{
    ...
    static uint32_t last_change_time = 0;  /* 上次状态变化时间 */
    uint32_t current_time = HAL_GetTick();

    phandle->hHighSpeedCapture = HAL_TIM_ReadCapturedValue(&htim4, TIM_CHANNEL_1);

    /* ✅ 关键修正：先保存当前状态到bPrevHallState，再读取新的HallState */
    /* 这样可以保证：bPrevHallState = 旧状态，HallState = 新状态 */
    phandle->bPrevHallState = phandle->HallState;  /* 先保存旧状态 */
    phandle->HallState = HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_8);  /* 再读新状态 */
    phandle->HallState |= HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_7) << 1;
    phandle->HallState |= HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_6) << 2;

    /* 检查霍尔状态是否改变（电机是否转动）*/
    uint8_t bStateChanged = (phandle->HallState != phandle->bPrevHallState);
    ...
    /* ✅ 超时检测：最后执行，超时则直接return，避免重新计算速度 */
    if (!bStateChanged && (current_time - last_change_time > 50)) {
        /* 50ms超时 → 电机停止 → 清零并退出 */
        phandle->HallSpeed = 0.0f;
        phandle->TempSpeed = 0.0f;
        phandle->AvrElSpeedDpp = 0.0f;
        phandle->hHighSpeedCapture = 0;
        phandle->Direction = 0;
        phandle->DeltaAngle = 0.0f;

        /* 调试输出：超时信息 */
        if (debug_cnt % 10 == 0) {
            extern UART_HandleTypeDef huart3;
            char buf[128];
            int len = sprintf(buf, "[TIMEOUT] current=%lu, last=%lu, diff=%lu\r\n",
                            current_time, last_change_time, current_time - last_change_time);
            HAL_UART_Transmit(&huart3, (uint8_t*)buf, len, 10);
        }

        return;  /* 直接返回，不执行后面的速度计算 */
    }
